...
/**
 *不同的SelectorProvider实现对应了不同的 选择器。由具体的SelectorProvider实现进行创建。
 * 另外说明一下，实际上netty底层也是通过这个设计获得具体使用的NIO模型
 *
 *
 */
...
public class NioSocketServer {
    ...
    /**
     ...
     实际上register(Selector sel, int ops, Object att)方法是ServerSocketChannel类的父类
     AbstractSelectableChannel提供的一个方法，表示只要继承了AbstractSelectableChannel
     类的子类都可以注册到选择器中。通过观察整个AbstractSelectableChannel继承关系，

     SelectionKey.OP_ACCEPT：不同的Channel对象可以注册的“我关心的事件”是不一样的
     例如ServerSocketChannel除了能够被允许关注OP_ACCEPT时间外，不允许再关心其他事件了
     （否则运行时会抛出异常）。以下梳理了常使用的AbstractSelectableChannel子类

     * @param args
     * @throws Exception
     */
   /* public static void main(String[] args) throws Exception {
       new NioSocketServer().mainD();
    }*/
}
